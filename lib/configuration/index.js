...
var SCHEMA_DEFAULTS = {
	...
	properties: {
		...
		dest: SCHEMA_DEST,
		configs: {
			description: 'Any property insides "configs" property considered a configuration property.',
			type: 'object',
...
function sort(taskInfo, rawConfig, parentConfig, optionalSchema) {
	var inheritedConfig, schema, subTaskConfigs, taskConfig, value;
	...
	// NOTE: A thought about that "configs" should be "normalized".
	// But remember that the "configs" and "$" property prefix are designed for tasks that have no schemas.
	// It just won't do anything try to normalize it without schema.
	taskConfig = regulate(taskConfig, ['configs']);
	assert(_.isPlainObject(taskConfig));

	subTaskConfigs = _.omit(inheritedConfig, Object.keys(taskConfig));
