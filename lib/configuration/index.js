...
function sort(taskInfo, rawConfig, parentConfig, optionalSchema) {
	...
	// NOTE: When there is `recipe`,  `plugin`,  `task`, `series` or `parallel` property,
	...
	value = _.omit(rawConfig, Object.keys(taskConfig));
	if (!optionalSchema && (taskInfo.recipe || taskInfo.plugin || taskInfo.task || taskInfo.series || taskInfo.parallel)) {
