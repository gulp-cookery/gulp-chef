...
function profile(fn, context, start, finish) {
	var result;

	try {
		start();
		result = fn.apply(context, asyncDone);
		if (result && typeof (result.on || result.subscribe || result.then) === 'function') {
			async(function () {
				return result;
			}, asyncDone);
			return result;
...
ConfigurableTaskFactory.prototype.create = function (prefix, taskInfo, taskConfig, recipe, tasks) {
	...
	function configurableTask(done) {
		var gulp, context, hrtime, branch;
		...
		branch = tasks && tasks.length > 0;

		return profile(recipe, context, start, finish);
