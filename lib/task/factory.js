...
ConfigurableTaskFactory.prototype.one = function (prefix, rawConfig, parentConfig) {
	...
	return simple() || auxiliary() || composite() || noop();
	...
	function auxiliary() {
		...
		function reference() {
			return recipes.reference(taskInfo);
		}

		function synthetic() {
			return recipes.stream(taskInfo) || recipes.flow(taskInfo);
		}
	}
	...
	function noop() {
		function recipe(done) {
			log("the task won't do anything, misspelled a recipe name?");
			done();
		}
		log('configure:', 'the task "' + taskInfo.name + "\" won't do anything, misspelled a recipe name?");
		return self.create(prefix, taskInfo, {}, recipe);
	}
