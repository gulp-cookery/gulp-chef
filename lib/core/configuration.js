...
function getTaskRuntimeInfo(nameValue) {
	var match, name, taskInfo;

	name = _.trim(nameValue);
...
function src(values) {
	...
	result = normalize(SCHEMA_SRC, values);
...
function dest(values) {
	...
	result = normalize(SCHEMA_DEST, values);
	return result.values;
}
...
function sort(taskInfo, rawConfig, parentConfig, optionalSchema) {
	...
	if (optionalSchema && _.size(optionalSchema)) {
		from(optionalSchema).to(taskInfo).imply(TASK_SCHEMA_MAPPINGS);
		schema = _.defaultsDeep({}, optionalSchema, SCHEMA_COMMONS);
	} else {
